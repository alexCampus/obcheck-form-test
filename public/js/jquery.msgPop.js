...
function initMsgPop()
{
	...
	var createLoadMore = function(container){
		if(loadMoreBtnCreated == false){
			...
			msgMoreBtn += '<span onclick="javascript:MsgPop.showMoreMessages();">=== Plus de notifications ===</span>';
			msgMoreBtn += '</div>';
	...
	var createCloseAll = function(container){
		if (closeAllBtnCreated == false) {
			...
			var msgDivCloseAll = '<div type="button" id="msgPopCloseAllBtn" onclick="MsgPop.closeAll()">Fermer les notifications</div>';
